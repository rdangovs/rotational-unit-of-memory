...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, isrum=False, lambda_=0):
    """
    Args:
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        def attention(decoder_state, coverage=None):
            ...
            with variable_scope.variable_scope("Attention"):
                ...
                context_vector = math_ops.reduce_sum(
                    array_ops.reshape(
                        attn_dist, [batch_size, -1, 1, 1]) * encoder_states,
        ...
        if lambda_ == 1:
            hidden_size = state.get_shape()[1].value
            e = tf.eye(hidden_size, batch_shape=[batch_size])
            e = tf.reshape(
                e, [batch_size, hidden_size * hidden_size])
            state = tf.concat([e, state], 1)
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            if pointer_gen:
                with tf.variable_scope('calculate_pgen'):
                    if isrum in ['dec', 'all']:
                        p_gen = linear(
                            [context_vector, state, x], 1, True)  # a scalar
                    else:
                        p_gen = linear(
