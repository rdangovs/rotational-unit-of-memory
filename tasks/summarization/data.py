...
# this may be true for CNN / DailyMail, but it is not true for ScienceDaily apparently
SENTENCE_START = '<s>'
SENTENCE_END = '</s>'
...
class Vocab(object):
    ...
    def __init__(self, vocab_file, max_size):
        """ Creates a vocab of up to max_size words, reading from the vocab_file. If Max_size is 0, reads the entire file.
        Args:
        ...
        with open(vocab_file, 'r') as vocab_f:
            for line in vocab_f:
                pieces = line.split()
                if len(pieces) != 2:
                    print('Warning : Incorrectly formated line in vocab file : %s\n' % line)
                    continue
                w = pieces[0]
                if w in [SENTENCE_START, SENTENCE_END, UNKNOWN_TOKEN, PAD_TOKEN, START_DECODING, STOP_DECODING]:
                    raise Exception("<s>, </s>, [UNK], [PAD], [START] and [STOP] shouldn\'t be in the vocab file, but %s is" %w)
                if w in self._word_to_id:
                    raise Exception("Duplicated Word in Vocab file %s" %w)
                self._word_to_id[w] = self._count
                self._id_to_word[self._count] = w
                self._count += 1
